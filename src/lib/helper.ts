...
export async function getContent(contentId: string) {
  try {
    const file = await fs.readFile(`./content/${contentId}.mdx`, "utf-8");
...
export async function listContents() {
  try {
    const contentFilePaths = (await fs.readdir("./content")).filter(
      (contentPath) => {
    ...
    for (const contentFilePath of contentFilePaths) {
      const file = await fs.readFile(`./content/${contentFilePath}`, "utf-8");
      const source = await serialize(file, { parseFrontmatter: true });
